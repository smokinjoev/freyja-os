...
# Install Freyja MacAgent as a user LaunchAgent on Iris/macOS.
...
if ! grep -Eq '^MACAGENT_TOKEN=.+$' "${PROJECT_DIR}/.env"; then
    echo "Error: MACAGENT_TOKEN must be set in ${PROJECT_DIR}/.env before installing MacAgent." >&2
    exit 1
fi
...
echo "Freyja MacAgent LaunchAgent installed and loaded."
echo "Plist: ${PLIST_DST}"
echo "Logs:  ${LOG_DIR}/macagent.log"
