...
JENNA_DOMAIN = SecurityDomain(
    ...
    display_name="JennaCide private domain",
    parent_domain_id=SecurityDomainId.HOUSEHOLD,
    allowed_domain_ids=frozenset({SecurityDomainId.HOUSEHOLD, SecurityDomainId.SYSTEM}),
    classification_floor=MemoryClassification.PRIVATE,
)
...
MACHINES: tuple[Machine, ...] = (
    ...
    Machine(
        ...
        security_domain_id=SecurityDomainId.HOUSEHOLD,
        capabilities=frozenset({"ollama", "lm-studio", "msty-nexus", "openai-compatible", "general.large", "code.large", "vision.large", "embeddings.local"}),
    ),
    Machine(
        machine_id="hera",
        display_name="Hera",
...
PERSISTENT_AGENTS: tuple[PersistentAgent, ...] = (
    ...
    PersistentAgent(
        ...
        tool_grants=frozenset({"web.search", "weather.current", "browser.control", "calendar.read", "calendar.write", "email.read", "messaging.send", "macagent.apple", "shell.run", "filesystem.read", "filesystem.write", "git.inspect", "git.write", "coding.execute", "documents.process", "vision.inspect", "music.control", "memory.private", "memory.shared", "system.health"}),
    ...
    PersistentAgent(
        ...
        tool_grants=frozenset({"web.search", "weather.current", "browser.control", "calendar.read", "calendar.write", "email.read", "messaging.send", "documents.process", "vision.inspect", "music.control", "memory.private", "memory.shared", "system.health"}),
    ...
    PersistentAgent(
        ...
        home_machine_id="atlas",
        aliases=frozenset({"legal-benedict", "paralegal-benedict", "paralegal"}),
        tool_grants=frozenset({"web.search", "browser.control", "documents.process", "vision.inspect", "memory.private", "system.health"}),
        private_memory_scope="enclave:paralegal",
        shared_memory_scopes=frozenset({"system"}),
        default_inference_capabilities=("legal_research", "document_review", "embeddings.local"),
        cloud_egress_policy_id="paralegal-local-only",
    ),
    PersistentAgent(
        agent_id="agent-47",
    ...
    PersistentAgent(
        ...
        home_machine_id="atlas",
        aliases=frozenset({"jenna", "jenna-agent"}),
        tool_grants=frozenset({"web.search", "weather.current", "browser.control", "calendar.read", "calendar.write", "email.read", "messaging.send", "vision.inspect", "music.control", "memory.private", "memory.shared", "system.health"}),
...
TOOL_CAPABILITIES: tuple[ToolCapabilityGrant, ...] = (
    ...
    ToolCapabilityGrant(tool_id="browser.control", category="browser", display_name="Browser", required_permission="tool:browser.control", protocol="mcp", machine_affinity="iris"),
    ToolCapabilityGrant(tool_id="calendar.read", category="calendar", display_name="Calendar read", required_permission="tool:calendar.read", protocol="mcp", machine_affinity="iris"),
    ToolCapabilityGrant(tool_id="calendar.write", category="calendar", display_name="Calendar write", required_permission="tool:calendar.write", protocol="mcp", mutation=True, machine_affinity="iris"),
    ToolCapabilityGrant(tool_id="email.read", category="email", display_name="Email read", required_permission="tool:email.read", protocol="mcp", machine_affinity="iris"),
    ToolCapabilityGrant(tool_id="messaging.send", category="messaging", display_name="Messaging", required_permission="tool:messaging.send", protocol="mcp", mutation=True, machine_affinity="iris"),
    ToolCapabilityGrant(tool_id="home-assistant.read", category="home_assistant", display_name="Home Assistant read", required_permission="tool:home_assistant.read", protocol="mcp", machine_affinity="atlas"),
    ToolCapabilityGrant(tool_id="home-assistant.control", category="home_assistant", display_name="Home Assistant control", required_permission="tool:home_assistant.control", protocol="mcp", mutation=True, machine_affinity="atlas"),
    ToolCapabilityGrant(tool_id="macagent.apple", category="macagent", display_name="Mac Agent", required_permission="tool:macagent.apple", protocol="mcp", machine_affinity="iris"),
    ...
    ToolCapabilityGrant(tool_id="documents.process", category="documents", display_name="Documents/PDF", required_permission="tool:documents.process", protocol="mcp"),
    ToolCapabilityGrant(tool_id="vision.inspect", category="vision", display_name="Vision", required_permission="tool:vision.inspect", protocol="mcp"),
    ToolCapabilityGrant(tool_id="music.control", category="music", display_name="Music", required_permission="tool:music.control", protocol="mcp", mutation=True, machine_affinity="iris"),
    ToolCapabilityGrant(tool_id="scheduling.create", category="scheduling", display_name="Scheduling", required_permission="tool:scheduling.create", protocol="mcp", mutation=True, machine_affinity="atlas"),
...
INFERENCE_ENDPOINTS: tuple[InferenceEndpoint, ...] = (
    InferenceEndpoint(
        endpoint_id="vulcan-nexus-fast",
        display_name="Vulcan Nexus fast local preset",
        provider="nexus",
        machine_id="vulcan",
        base_url="http://100.94.80.21:3939",
        model="@preset/freyja-fast-local",
        capabilities=frozenset({"route.fast", "general.local", "chat", "summarization"}),
        security_domain_id=SecurityDomainId.HOUSEHOLD,
        priority=8,
    ),
    InferenceEndpoint(
        endpoint_id="vulcan-nexus-strong",
        display_name="Vulcan Nexus strong local preset",
        provider="nexus",
        machine_id="vulcan",
        base_url="http://100.94.80.21:3939",
        model="@preset/freyja-strong-local",
        capabilities=frozenset({"route.general", "general.large", "reasoning"}),
        security_domain_id=SecurityDomainId.HOUSEHOLD,
        priority=8,
    ),
    InferenceEndpoint(
        endpoint_id="vulcan-nexus-coder",
        display_name="Vulcan Nexus coder preset",
        provider="nexus",
        machine_id="vulcan",
        base_url="http://100.94.80.21:3939",
        model="@preset/freyja-coder",
        capabilities=frozenset({"route.code", "code.large", "coding"}),
        security_domain_id=SecurityDomainId.HOUSEHOLD,
        priority=8,
    ),
    InferenceEndpoint(
        endpoint_id="vulcan-nexus-vision-docs",
        display_name="Vulcan Nexus vision and documents preset",
        provider="nexus",
        machine_id="vulcan",
        base_url="http://100.94.80.21:3939",
        model="@preset/freyja-vision-docs",
        capabilities=frozenset({"route.vision", "vision.large", "document_review"}),
        security_domain_id=SecurityDomainId.HOUSEHOLD,
        priority=8,
    ),
    InferenceEndpoint(
    ...
    InferenceEndpoint(
        ...
        display_name="Vulcan 32B general reasoning endpoint",
        provider="ollama",
        machine_id="vulcan",
        base_url="http://100.94.80.21:11434",
        model="qwen2.5:32b-instruct",
        capabilities=frozenset({"route.general", "general.large", "chat", "reasoning"}),
        security_domain_id=SecurityDomainId.HOUSEHOLD,
        priority=20,
    ),
    InferenceEndpoint(
        endpoint_id="vulcan-deep",
        display_name="Vulcan sleep-on-it deep reasoning endpoint",
        provider="ollama",
        machine_id="vulcan",
        base_url="http://100.94.80.21:11434",
        model="qwen2.5vl:72b",
        capabilities=frozenset({"route.deep", "general.deep", "reasoning.deep", "long_context"}),
        security_domain_id=SecurityDomainId.HOUSEHOLD,
        priority=30,
    ),
    InferenceEndpoint(
    ...
    InferenceEndpoint(
        ...
        base_url="http://100.94.80.21:11434",
        model="qwen2.5vl:72b",
        capabilities=frozenset({"route.vision", "vision.large"}),
        security_domain_id=SecurityDomainId.HOUSEHOLD,
        priority=20,
    ),
    InferenceEndpoint(
        ...
        capabilities=frozenset({"route.embedding", "embeddings.local"}),
        security_domain_id=SecurityDomainId.HOUSEHOLD,
        priority=10,
    ),
    InferenceEndpoint(
    ...
    InferenceEndpoint(
        endpoint_id="benedict-paralegal-nexus",
        display_name="Benedict paralegal Nexus local preset",
        provider="nexus",
        machine_id="vulcan",
        base_url="http://100.94.80.21:3939",
        model="@preset/benedict-paralegal-local",
        capabilities=frozenset({"route.private", "route.vision", "route.embedding", "chat", "document_review", "legal_research", "embeddings.local"}),
        security_domain_id=SecurityDomainId.PARALEGAL,
        priority=8,
    ),
    InferenceEndpoint(
