...
The Freyja Director is not part of this stack. The connector forwards
authorized requests to the Atlas Director using `FREYJA_DIRECTOR_URL`. Vulcan
provides the primary complex `local_reasoning` and coding models over
Tailscale, while Iris remains available for Apple integration, MacAgent access,
and lightweight local routing/inference. The Director must handle provider
failure through configured fallback or explicit provider failure.
...
Atlas Director endpoint. Set `FREYJA_CONNECTOR_TOKEN` to the same strong value
configured on the Director. The populated `.env` must remain mode
