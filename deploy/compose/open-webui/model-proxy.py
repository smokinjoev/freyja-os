...
class Handler(BaseHTTPRequestHandler):
    ...
    def _proxy_models(self) -> None:
        ...
        for base_url, api_key in ((PRIMARY_BASE_URL, PRIMARY_API_KEY), (FALLBACK_BASE_URL, FALLBACK_API_KEY)):
            status, headers, body = self._upstream(base_url, api_key, "GET", "/models", b"")
            if status >= 400:
                continue
            payload = json.loads(body.decode("utf-8"))
            for model in payload.get("data", []):
                model_id = model.get("id")
                if model_id in APPROVED_MODELS or model_id in FALLBACK_MODELS:
    ...
    def _proxy(self) -> None:
        ...
        if self.command == "POST" and self.path.split("?", 1)[0] == "/v1/chat/completions":
            if requested_model not in APPROVED_MODELS and requested_model not in FALLBACK_MODELS:
                self._send(
                    400,
                    {"content-type": "application/json"},
    ...
    def _model_available(self, base_url: str, api_key: str, model_id: str) -> bool:
        status, _, body = self._upstream(base_url, api_key, "GET", "/models", b"")
        if status >= 400:
            return False
        try:
            payload = json.loads(body.decode("utf-8"))
    ...
    def _upstream(self, base_url: str, api_key: str, method: str, path: str, body: bytes) -> tuple[int, dict[str, str], bytes]:
        request = urllib.request.Request(
            f"{base_url}{path}",
            data=body or None,
            method=method,
