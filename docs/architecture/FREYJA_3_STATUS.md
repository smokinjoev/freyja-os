...
| --- | --- | --- |
| Iris | `iris.lan`, macOS 26.5.2, Director and iMessage connector running locally; live iMessage runtime source refreshed from the canonical checkout with no source drift; `FREYJA3_CANONICAL_ENABLED=true` loaded by the local Director | Apple-side runtime active. MacAgent and iMessage path available. Apple-side `/canonical/route` now enters the Freyja 3 gateway/runtime path. Synthetic iMessage file/photo routing returns HTTP 200 after restart. Family identity mapping still needs the four person-to-iMessage-address assignments before Beth/Liam/Jenna route checks can pass. |
...
- Extend concrete ingestion workers beyond bounded text/path observations into specific source adapters as needed.
