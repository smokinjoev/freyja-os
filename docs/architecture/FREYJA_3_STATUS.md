...
- Added a `freyja3-worker-runner` CLI plus reproducible user-level systemd service/timer assets so Mars-style hosts can poll Atlas for one queued worker job per run and complete lightweight monitoring jobs.
...
- Install and verify the recurring Mars `freyja3-worker-runner.timer` on top of the durable Freyja 3 worker job API.
- Add concrete ingestion pipelines on top of the worker runner.
